...
> The rationales below were written on 2026-09-16 from the advisory text and the
> dependency paths in `pnpm audit --json`, and reconciled with the point-in-time audit
> report in [`docs/security/dependency-audit.md`](docs/security/dependency-audit.md)
> (last reviewed 2026-08-28). That report is the narrative record of a single audit;
> this file is the standing list that must match `pnpm.auditConfig` at all times.
...
- **2026-09-16 — `CVE-2025-13204` / `GHSA-8gw3-rxh4-v6jx` and `CVE-2025-12735` /
  `GHSA-jc85-fpwf-qm7x` (expr-eval).** Closed by swapping the calculator's parser from
  the unmaintained `expr-eval` to `expr-eval-fork@3.0.3` (MIT,
  ...
  `GHSA-8gw3-rxh4-v6jx` and 3.0.1 for `GHSA-jc85-fpwf-qm7x`. Both CVEs were removed from
  `pnpm.auditConfig.ignoreCves` at the same time.
