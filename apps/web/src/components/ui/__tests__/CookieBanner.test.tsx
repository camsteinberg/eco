...
describe("CookieBanner", () => {
  ...
  afterEach(() => {
    // The composer-reserve flag lives on <html>; clear it so it can't leak
    // between cases (RTL unmount already runs the effect cleanup, but be explicit).
  ...
  it("positions away from chat composer send controls", () => {
    ...
    expect(banner).toHaveClass("lg:bottom-6", "lg:right-6", "lg:left-auto");
  });
  ...
  it("does NOT flag <html> outside the chat surface (no composer to clear)", () => {
    render(<CookieBanner />);
