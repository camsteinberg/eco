...
describe('runLocalRuntimeSelfHeal', () => {
  ...
  it('removes an in-progress download marker older than 5 minutes', () => {
    ...
    const key = `${IN_PROGRESS_PREFIX}local/phi3-mini-4k-q4f16`;
  ...
  it('only touches keys with the in-progress prefix', () => {
    ...
    localStorage.setItem('eco-model-downloaded-local/phi3', '{"ok": true}');
    localStorage.setItem(
      `${IN_PROGRESS_PREFIX}local/phi3-mini-4k-q4f16`,
    ...
    expect(localStorage.getItem('eco-model-downloaded-local/phi3')).toBe('{"ok": true}');
    expect(localStorage.getItem(`${IN_PROGRESS_PREFIX}local/phi3-mini-4k-q4f16`)).toBeNull();
  });
